...
encoded_columns = ohe.get_feature_names_out(['Month', 'Hour-Minute'])

df = pd.concat([df, pd.DataFrame(ohe_result, columns=encoded_columns)], axis=1)
...
def create_ts_dataset(dataset_x, dataset_y, prediction_interval, window_size, sliding_size):
    x = []
    y = []
    for i in range(0, len(dataset_x) - window_size -  prediction_interval, sliding_size):
        
        x.append(dataset_x[i:i + window_size])
        
        y.append(dataset_y[i + window_size +  prediction_interval - 1])

    return np.array(x), np.array(y) # x verileri 3 boyutlu oluyor 


scaled_training_dataset_x, training_dataset_y = create_ts_dataset(raw_scaled_training_dataset_x, 
                                                                  raw_training_dataset_y, 
                                                                  PREDICTION_INTERVAL, 
                                                                  WINDOW_SIZE, 
                                                                  SLIDING_SIZE)


scaled_test_dataset_x, test_dataset_y = create_ts_dataset(raw_scaled_test_dataset_x, 
                                                          raw_test_dataset_y, 
                                                          PREDICTION_INTERVAL, 
                                                          WINDOW_SIZE, 
                                                          SLIDING_SIZE)
